...
drawPlayerIcons_thread = [] spawn
{
	...
	_allowWeaponThermal = ["A3W_allowThermalInWeapon"] call isConfigOn;
	_allowBinocularThermal = ["A3W_allowThermalInBinocular"] call isConfigOn;
	...
	waitUntil
	{
		...
		if (!visibleMap && isNull findDisplay 49) then
		{
			...
			if !(_allowWeaponThermal) then
			{
				_thermalActive = currentVisionMode player isEqualTo 2;
				if (_thermalActive || !isNil "A3W_thermalWeaponOffline") then
				{
					_weapon = currentWeapon player;
					_thermalBinoculars = (_allowBinocularThermal && (getText (configfile >> "CfgWeapons" >> _weapon >> "nameSound") isEqualTo "binoculars"));
					_ownWeapon = true;
					...
					if (_thermalActive &&{!_thermalBinoculars && cameraOn == vehicle player && _weapon in weapons player && _ownWeapon &&
